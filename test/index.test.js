...
describe('real life scenario', function() {
  before(function() {
    const mockSubflows = [
      'City Attraction',
      'Prepaid', 'Postpaid', 'Cardless',
      'DOTW', 'Local', 'Expedia', 'Hotel Beds',
      'Firebase User', 'Auth0 User',
    ];

    mockSubflows.forEach(mockSubflow => {
      reflow.subflow(mockSubflow, () => ({
        before() {},
        suites: [reflow.getSuite('NOOP')],
      }))
    })

    reflow.subflow('Cardless Suppliers', function() {
      return {
        condition: branches => {
          return branches
            .filter(branch => branch.type === "subflow")
            .find(branch => branch.name === "Cardless")
        },
        suites: [
          reflow.getSubflow('Local'),
        ],
      }
    });

    reflow.subflow('Postpaid Suppliers', function() {
      return {
        condition: branches => {
          return branches
            .filter(branch => branch.type === "subflow")
            .find(branch => branch.name === "Postpaid")
        },
        suites: [
          reflow.fork([
            reflow.getSubflow('Local'),
            reflow.getSubflow('Expedia'),
          ]),
        ],
      }
    });

    reflow.subflow('Prepaid Suppliers', function() {
      return {
        condition: branches => {
          return branches
            .filter(branch => branch.type === "subflow")
            .find(branch => branch.name === "Prepaid")
        },
        suites: [

          reflow.fork([
            reflow.getSubflow('DOTW'),
            reflow.getSubflow('Local'),
            reflow.getSubflow('Expedia'),
            reflow.getSubflow('Hotel Beds'),
          ])
        ],
      }
    });

    reflow.subflow('Coupon Redemption Auth0 User', function() {
      return {
        condition: branches => {
          return branches
            .filter(branch => branch.type === "subflow")
            .find(branch => branch.name === "Auth0 User")
        },
        suites: [reflow.getSuite('Coupon Redemption API Auth0')],
      }
    });

    reflow.subflow('Coupon Redemption Firebase User', function() {
      return {

        condition: branches => {
          return branches
            .filter(branch => branch.type === "subflow")
            .find(branch => branch.name === "Firebase User")
        },
        suites: [reflow.getSuite('Coupon Redemption API for Firbase Users')],
      }
    });
  ...
  it('just works', function() {
    ...
    const flow = reflow("Mobile", function() {
      return {
        suites: [
          reflow.getSubflow('City Attraction'),
          reflow.fork([
            reflow.getSubflow('Prepaid'),
            reflow.getSubflow('Postpaid'),
            reflow.getSubflow('Cardless'),
          ]),
          reflow.getSubflow('Prepaid Suppliers'),
          reflow.getSubflow('Postpaid Suppliers'),
          reflow.getSubflow('Cardless Suppliers'),
          reflow.fork([
            reflow.getSubflow('Firebase User'),
            reflow.getSubflow('Auth0 User'),
          ]),
          reflow.getSubflow('Coupon Redemption Auth0 User'),
          reflow.getSubflow('Coupon Redemption Firebase User'),
        ],
      }
    })
    // console.log("flow::", JSON.stringify(flow,2,2))
    // console.log("flow::", flow.length)
    const analysisTree = analyzeTree('Mobile', flow);
    console.log(analysisTree.join('\n'))
