...
const beforeAllBefores = function (title, fn) {
  if (this.isPending()) {
    return this;
  }
...
module.exports = Mocha.interfaces['reflow-bdd'] = function (suite) {
  ...
  suite.on('pre-require', function (context, file, mocha) {
    ...
    context.describe = context.context = function (title, fn) {
      ...
      let hookok = false
      hooks.length && console.log(newSuite._beforeAll);
      while(hooks.length) {
        hookok = true;
        var {
          name,
        ...
        before && beforeAllBefores.call(newSuite, name, before);
        // after && newSuite.afterAll(name, after);
      }
      if(hookok) {
        console.log('newSuite::', newSuite._beforeAll)
      }
